...
def safety(moves):
    ...
    a1 = [(sum(b[12:15]) if e else (-sum(b[19:21])+b[21]), b, e)
          for b, e in moves]
    m = max(a1)[0]
...
nicks = ["Donkey", "Extra", "bear", "hit", "Chuck",
         "Frank", "safe", "homestretch", "party"]
...
def bestHumanStrategySoFar(m): return chainFilt(
    m, [strategies[i] for i in (3, 6, 1, 8, 2, 7, 5, 4, 0)])
# (3, 1, 6, 5, 2, 4, 0, 7)])
